...
create policy "Select open groups or groups I belong to"
...
create policy "View memberships for my groups"
...
create policy "Read messages from my groups"
  on public.group_messages for select to authenticated
  using (public.is_group_member(auth.uid(), group_id));

create policy "Send messages to my groups"
  on public.group_messages for insert to authenticated
  with check (
    user_id = auth.uid() and public.is_group_member(auth.uid(), group_id)
  );

create policy "Delete my own messages"
